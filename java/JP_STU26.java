...
public class JP_STU26 extends BFrame implements ActionListener{
	Date d = new Date();
	...
	public JP_STU26() {
		this.setTitle("26기 일본어 학생 목록");
		list = dao.alljp_26();
		init();
	}
	
	public JP_STU26(ArrayList<jp26Bean> list){
		this.setTitle("26기 일본어 학생 목록");
		this.list = list;
		init();
	}
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getActionCommand().equals("새로고침")){
			...
			list = dao.insertJp_26();
			try {
				if (date.format(d).equals(bean.getDate())) {
					dao.delete_jp26(date.format(d));
				}
